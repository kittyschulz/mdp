...
class Planner:
    ...
    def value_iteration(self, gamma=1.0, n_iters=1000, theta=1e-10):
        ...
        delta_track = np.zeros((n_iters, len(self.P)), dtype=np.float64)
        reward_track = np.zeros(n_iters, dtype=np.float64)
        ...
        while i < n_iters-1 and not converged:
            ...
            delta_track[i] = np.max(np.abs(V - np.max(Q, axis=1)))
            reward_track[i] = np.mean(np.max(Q, axis=1))
        ...
        return V, V_track, delta_track, reward_track, pi
    ...
    def policy_iteration(self, gamma=1.0, n_iters=50, theta=1e-10):
        ...
        V = np.zeros(len(self.P), dtype=np.float64)
        V_track = np.zeros((n_iters, len(self.P)), dtype=np.float64)
        delta_track = np.zeros(n_iters, dtype=np.float64)
        reward_track = np.zeros(n_iters, dtype=np.float64)
        i = 0
        converged = False
        while i < n_iters and not converged:
            ...
            V, delta, reward = self.policy_evaluation(pi, V, gamma, theta)
            V_track[i-1] = V
            delta_track[i-1] = delta
            reward_track[i-1] = reward
    ...
    def policy_evaluation(self, pi, prev_V, gamma=1.0, theta=1e-10):
        ...
        while True:
            ...
            for s in range(len(self.P)):
                ...
                V[s] += v
                delta = max(delta, abs(prev_V[s] - V[s]))
                reward_track += v
            if np.max(np.abs(prev_V - V)) < theta:
                break
            prev_V = V.copy()
        return V, delta, reward_track
